...
static void compile_exprs(STATE, jz_parse_node* node, jz_bool value);
...
static void compile_logical_binop(STATE, jz_parse_node* node, jz_bool value);
...
static void compile_statement(STATE, jz_parse_node* node) {
  ...
  case jz_parse_exprs:
    compile_exprs(jz, state, node, jz_false);
    break;
...
void compile_return(STATE, jz_parse_node* node) {
  if (node == NULL)
    ...
    compile_exprs(jz, state, node, jz_true);
...
void compile_if(STATE, jz_parse_node* node) {
  ...
  compile_exprs(jz, state, CAR(node).node, jz_true);
...
void compile_do_while(STATE, jz_parse_node* node) {
  ...
  jz_bool skip_conditional = conditional_literal != NULL &&
...
void compile_while(STATE, jz_parse_node* node) {
  ...
  /* If the conditional is NULL or a literal value that evaluates to true,
     we don't bother to check it.
     This makes stuff like "for (;;)" and "while (jz_true)" faster. */
  jz_bool skip_conditional = CAR(node).node == NULL ||
...
jz_ptrdiff_vector* compile_switch_conditionals(STATE, jz_parse_node* node) {
  ...
  while (node != NULL) {
    ...
    compile_exprs(jz, state, CAR(case_node).node, jz_true);
...
void compile_exprs(STATE, jz_parse_node* node, jz_bool value) {
  jz_bool first = jz_true;

  while (node != NULL) {
    int old_cap = first ? 0 : state->stack_length;
    jz_bool this_value = jz_false;
...
#define SIMPLE_UNOP_CASE(operator, opcode)              \
  case operator: {                                      \
    compile_expr(jz, state, CDR(node).node, jz_true);      \
    PUSH_OPCODE(opcode);                                \
                                                        \
    if (!value)                                         \
      PUSH_OPCODE(jz_oc_pop);                           \
...
static void compile_unit_shortcut(STATE, jz_parse_node* node,
  ...
  variable var = compile_identifier(jz, state, CDR(node).node, jz_true);
...
void compile_logical_binop(STATE, jz_parse_node* node, jz_bool value) {
  int left_cap, right_cap;
  ptrdiff_t jump;

  compile_expr(jz, state, CADR(node).node, jz_true);
...
void compile_simple_binop(STATE, jz_parse_node* node, jz_opcode op, jz_bool value) {
  ...
  compile_expr(jz, state, CDDR(node).node, jz_true);
  right_cap = state->stack_length;
...
void compile_call(STATE, jz_parse_node* node, jz_bool value) {
  ...
  compile_expr(jz, state, CAR(node).node, jz_true);
  cap = state->stack_length;

  while (arg != NULL) {
    compile_expr(jz, state, CAR(arg).node, jz_true);
